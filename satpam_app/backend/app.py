...
# Database Configuration (Saved to db_folder)
basedir = os.path.abspath(os.path.dirname(__file__))
db_dir = os.path.join(basedir, '../db_folder')
os.makedirs(db_dir, exist_ok=True)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + os.path.join(db_dir, 'satpam.db')
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False

db.init_app(app)

# Make sure keys exist
key_path = os.path.join(basedir, 'keys/server_private.pem')
if not os.path.exists(key_path):
    print("Warning: RSA Keys not found! Run generate_keys.py first.")

crypto = None
if os.path.exists(key_path):
    crypto = HybridCrypto(key_path)
...
@app.route('/api/lookup_npm/<npm>', methods=['GET'])
def lookup_npm(npm):
    nama = STUDENT_DB.get(npm)
    if nama:
        return jsonify({"status": "success", "nama": nama}), 200
    return jsonify({"status": "error", "message": "NPM tidak ditemukan dalam database."}), 404

@app.route('/api/get_students', methods=['GET'])
def get_students():
    # Return list of dicts for frontend dropdown
    data = [{"npm": k, "nama": v} for k, v in STUDENT_DB.items()]
    # Sort by NPM
    data.sort(key=lambda x: x["npm"])
